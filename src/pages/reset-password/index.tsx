...
import { usePostApiAuthResetPassword } from '@/gen/sneakverse/hooks/usePostApiAuthResetPassword'
...
function ResetPassword() {
  ...
  const onSubmit = (values: FormValues) => {
    resetMutation.mutate(
      { data: { email: values.email, token: values.otp, newPassword: values.password } },
      {
        onSuccess: () => {
          reset()
        ...
        onError: (err) => {
          ...
          setError('otp', { type: 'server', message: String(msg) })
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen px-4">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-[0_20px_40px_-12px_rgba(0,0,0,0.25)] border border-black/5">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-6 pb-8 space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest">OTP Code</label>
              <input
                type="text"
                maxLength={6}
                className={cn(
                  "w-full h-11 rounded-lg border border-black/10 bg-[#f6f5f2] px-3 outline-none focus:ring-2 focus:ring-[#ff6b6b] transition-all tracking-[0.5em] text-center font-bold text-lg",
                  errors.otp && "border-[#ff6b6b] focus:ring-[#ff6b6b]"
                )}
                placeholder="000000"
                {...register('otp')}
              />
              {errors.otp && (
                <p className="text-[#ff6b6b] text-xs">{errors.otp.message}</p>
              )}
            </div>
            ...
            <div className="text-center text-sm text-black/70">
              Remember your password? <a className="font-bold text-blue-500 relative group inline-block cursor-pointer" onClick={() => navigate({ to: '/login' })}>
                Login
        ...
        {isSuccessOpen && (
          <div className="absolute inset-0 z-[9999] flex items-center justify-center bg-black/40">
            <div className="w-full max-w-sm rounded-2xl bg-white shadow-[0_20px_40px_-12px_rgba(0,0,0,0.25)] border border-black/5 p-6 text-center">
              ...
              <p className="mt-2 text-sm text-black/70">Your password has been reset successfully.</p>
              <div className="mt-6 flex justify-center gap-3">
                ...
                <button
                  type="button"
                  className="px-4 h-10 rounded-lg border border-black/10 text-sm font-medium cursor-pointer hover:bg黑/5 transition"

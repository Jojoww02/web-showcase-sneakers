...
import { usePostApiAuthLogout } from '@/gen/sneakverse/hooks/usePostApiAuthLogout';
...
const CardNav: React.FC<CardNavProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    if (isProfileOpen) {
      ...
      gsap.to(el, { autoAlpha: 0, y: 8, scale: 0.98, duration: 0.2, ease: 'power3.in' });
    }
  }, [isProfileOpen]);
  ...
  return (
    ...
    >
      <nav
        ...
        style={{ backgroundColor: isExpanded ? baseColor : '#141414', overflow: isProfileOpen ? 'visible' : 'hidden' }}
      >
          ...
          {token ? (
            <div className="relative hidden md:inline-block">
              <button
                ...
                type="button"
                className="relative inline-flex items-center h-full px-3 rounded-full cursor-pointer"
                aria-label="Profile"
              ...
              <div
                ref={profileMenuRef}
                className={`absolute right-0 mt-2 w-40 z-[9999] rounded-lg bg-neutral-900/95 ring-1 ring-white/10 backdrop-blur-sm shadow-lg ${isProfileOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}
                style={{ transformOrigin: 'top right', visibility: isProfileOpen ? 'visible' : 'hidden' }}
              >
                <button
                  type="button"
                  className="block w-full text-left px-3 py-2 text-white/90 hover:text-white hover:bg-white/10 transition cursor-pointer"
      ...
      {isLogoutModalOpen && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
          <div className="w-full max-w-sm rounded-xl bg-neutral-900 border border-white/10 p-6 shadow-2xl">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10 transition cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 rounded-lg text-sm font-medium bg-white text-black hover:bg-white/90 transition cursor-pointer"
              >
                Yes, Logout
              </button>
            </div>
